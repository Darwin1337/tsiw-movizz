<template>
  <div class="mt-3" v-if="!loading.prizes">
    <div>
      <button class="addNewQuizButton" data-bs-toggle="modal" data-bs-target="#addPrizeModal">Add new prize</button>
    </div>
    <div class="row">
      <div class="col-xl-2 col-lg-3 col-md-4 col-12 mt-3 quizImg" v-for="i in (data.prizes.length >= mostrar ? mostrar : data.prizes.length)" :key="i">
        <img class="w-100" style="border-radius:5px 5px 0px 0px; max-height:180px;" :src="data.prizes[i - 1].image" alt="Prize image">
        <div class="d-flex flex-column" style="padding:10px; background-color:var(--azul-escuro2); border-radius:0px 0px 5px 5px;">
          <p class="m-0 text-center" style="white-space: nowrap; text-overflow: ellipsis; overflow: hidden;">{{data.prizes[i - 1].name}}</p>
          <button class="editQuizButtonPrizes" data-bs-toggle="modal" data-bs-target="#editPrizeModal" @click="listInfo(data.prizes[i - 1].prize_id)">Edit</button>
          <button class="removeQuizButton" @click="removePrize(data.prizes[i - 1].prize_id)">Remove</button>
        </div>
      </div>
      <div v-if="mostrar < data.prizes.length" class="w-100 d-flex justify-content-center mt-4">
        <button class="rounded-btn" @click="mostrar = mostrar + 12 <= data.prizes.length ? mostrar + 12 : data.prizes.length">Load More</button>
      </div>
    </div>
    <!-- Remove prize modal -->
      ...
      <div class="modal-dialog modal-dialog-centered" style="max-width:400px;">
        <div class="modal-content modalrewards">
          <div class="modal-body m-0 gy-3">
            <div class="d-flex h-100 flex-column justify-content-between">
              <h5>Edit prize</h5>
              <form @submit.prevent="editPrizeInfo(editPrizes.prizeId).id" id="addNewQuizModal">
                <p class="m-0 mt-3 colorOrange">General</p> 
                <div class="mt-3">
...
export default {
  ...
  methods: {
    ...mapActions(["getAllPrizes", "addPrize", "deletePrize", "editPrize"]),
    async getPrizes() {
      try {
        this.loading.prizes = true;
        this.data.prizes = await this.getAllPrizes();
        if (this.data.prizes.success) {
          this.data.prizes = this.data.prizes.msg;
    ...
    async removePrize(id){
      ...
      if (check.isConfirmed) {
        try {
          const res = await this.deletePrize(id);
          if (res.success) {
            this.data.prizes = this.data.prizes.filter(prize => prize.prize_id != id);
            this.$store.state.loggedUserData.data.prizes_reedemed = this.$store.state.loggedUserData.data.prizes_reedemed.filter(prize => prize.prize_id.prize_id != id);
    ...
    async addNewPrize() {
      if (this.prizes.prizeCost > 0) {
        try {
          const res = await this.addPrize({ name: this.prizes.prizeName, image: this.prizes.prizeImg, price: this.prizes.prizeCost });
          if (res.success) {
            this.data.prizes.push(res.data);
        ...
        this.$swal('Error!', 'The price entered is invalid!', 'error');
      }
    },
    async editPrizeInfo(id){
      if (this.editPrizes.prizeName.trim() && this.editPrizes.prizeCost > 0 && this.editPrizes.prizeImg.trim()) {
        
        try {
          const res = await this.editPrize({ prize_id: id, data: { name: this.editPrizes.prizeName, price: this.editPrizes.prizeCost, image: this.editPrizes.prizeImg } });
          if (res.success) {
            const prizeIdx = this.data.prizes.findIndex(a => a.prize_id == id);
            this.data.prizes[prizeIdx].name = this.editPrizes.prizeName;
            this.data.prizes[prizeIdx].price = this.editPrizes.prizeCost;
            this.data.prizes[prizeIdx].image = this.editPrizes.prizeImg;

            this.$store.state.loggedUserData.data.prizes_reedemed.forEach((prize, i) => {
              if (prize.prize_id.prize_id.toString() == id.toString()) {
                this.$store.state.loggedUserData.data.prizes_reedemed[i].prize_id.name = this.editPrizes.prizeName;
                this.$store.state.loggedUserData.data.prizes_reedemed[i].prize_id.price = this.editPrizes.prizeCost;
                this.$store.state.loggedUserData.data.prizes_reedemed[i].prize_id.image = this.editPrizes.prizeImg;
              }
            });
        ...
        this.$swal.fire({ icon: 'error', title: 'Error!', text: 'Some fields were left empty!' });
      }
    },
    listInfo(id){
      const prizeData = this.data.prizes.find(a => a.prize_id == id);
      this.editID = id;
      this.editPrizes.prizeId = id;
      this.editPrizes.prizeName = prizeData.name;
      this.editPrizes.prizeCost = prizeData.price;
      this.editPrizes.prizeImg = prizeData.image;
    },
    performClick(name) {
