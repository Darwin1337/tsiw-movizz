        ...
        <div class="row h-100 w-100 pt-5 pb-5 m-0">
          <div class="poster col-lg-4 m-auto">
            <div class="w-100 poster-wrapper">
              <img id="title-poster" :src="webpSupported ? require('../assets/images/content/' + data.title.poster_webp) : data.title.poster"
                alt="Title cover" />
            </div>
          </div>
          <div class="main-info col-lg-8">
            ...
            <div>
              <div class="d-flex align-items-center mb-2" style="gap: 20px">
                ...
                <a class="m-0 d-flex align-items-center" style="gap: 15px"
                  :href="data.title.trailer" target="_blank">
            ...
            <p id="title-running-time">{{ data.title.duration == "Running time not available" ? "Running time not available" : data.title.duration + " minutes" }}</p>
            ...
            <div class="
              ...
              ">
              <div>
                <p class="orange-btn text-center mb-3">IMDb {{parseFloat(data.title.imdb_rating).toFixed(1)}}</p>
          ...
          <template v-for="i in data.title.cast.length">
            <div v-show="(i - 1) > 5 ? (showFullCast ? true : false) : true" class="cast-card col-lg-2 col-md-3 col-sm-4 col-6 g-2" :key="i">
              <div style="background-color: var(--azul-escuro2); border-radius: 10px; w-100">
                <div class="cast-photo" :style="{ backgroundImage: 'url(' + (webpSupported ? require('../assets/images/content/' + data.title.cast[i - 1].image_webp) : data.title.cast[i - 1].image) + ')' }"
                  style="
                  ...
                  "></div>
                <p class="text-center pt-3 m-0" style="white-space: nowrap; text-overflow: ellipsis; overflow: hidden;">{{ data.title.cast[i - 1].name }}</p>
                <div class="w-100 pb-3 d-flex justify-content-center m-0 text-center">
                  <p class="p-0 m-0" style="color: var(--cinza2); width: 90%; white-space: nowrap; text-overflow: ellipsis; overflow: hidden;">{{ fixCharacterName(data.title.cast[i - 1].asCharacter) }}</p>
                </div>
              </div>
            </div>
          </template>
        ...
        <section id="seasons">
          ...
          <div class="d-flex num-seasons" style="gap: 25px">
            <p class="
                ...
                align-items-center" :class="selectedSeason == i ? 'active' : ''" v-for="i in data.title.seasons" :key="i"
          ...
          <div class="carousel-wrapper" style="position: relative;">
            ...
            <div class="row-carousel disable-scrollbars">
              ...
              <div class="tile" v-for="(episodio, i) in data.title.episodes.filter(ep => ep.num_temporada == selectedSeason)" :key="i">
                <div class="tile__media">
                  <img class="tile__img" :src="webpSupported ? require('../assets/images/content/' + episodio.banner_webp) : episodio.banner" />
                </div>
                <div class="tile__details">
                  <div class="tile__title">
                    <strong>{{ episodio.episode }}</strong>&nbsp;&nbsp;&nbsp;"{{ episodio.title }}"
                  </div>
                </div>
              </div>
            </div>
