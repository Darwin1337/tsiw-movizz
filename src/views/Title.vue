    ...
    <div v-if="!loading.title">
            ...
            <div class="container h-100">
                    ...
                    <div class="main-info col-lg-8">
                        ...
                        <div>
                            <div class="d-flex align-items-center mb-2" style="gap: 20px;">
                                ...
                                <a class="m-0 d-flex align-items-center" style="gap: 15px;" :href="data.title.trailer" target="_blank">
                ...
                "
                        ...
                        <p id="title-running-time">
                            {{ data.title.duration == "Running time not available" ? "Running time not available" : data.title.duration + " minutes" }}
                        </p>
                        ...
                        <div class="title-actions-wrapper d-flex justify-content-between flex-wrap">
                            <div>
                            ...
                            <div class="title-actions d-flex align-items-center flex-wrap" style="gap: 15px; flex-grow: 0;">
                                <button class="blur-btn" style="font-weight: 500;" id="like" @click="addNewLike()">Add to favourites <i class="fas fa-heart"></i></button>
                                <button class="blur-btn" style="font-weight: 500;" id="seen" @click="addNewSeen()">Mark as seen <i class="fas fa-eye"></i></button>
                                <div class="wrapper-stars position-relative d-flex justify-content-center">
                                    ...
                                    <button class="blur-btn" style="font-weight: 500;" @click="showStars = !showStars">Rate <i class="fas fa-star"></i></button>
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        <div class="container mb-5 mt-5">
            ...
            <section id="cast" class="mt-5 mb-3">
                ...
                <div class="cast-members row">
                    <template v-for="i in data.title.cast.length">
                        <div v-show="i - 1 > 5 ? (showFullCast ? true : false) : true" class="cast-card col-lg-2 col-md-3 col-sm-4 col-6 g-2">
                    ...
                    backgroundImage:
                      ...
                      ')',
                                ...
                                ></div>
                                <p class="text-center pt-3 m-0" style="white-space: nowrap; text-overflow: ellipsis; overflow: hidden;">
                                    {{ data.title.cast[i - 1].name }}
                                </p>
                                <div class="w-100 pb-3 d-flex justify-content-center m-0 text-center">
                                    <p class="p-0 m-0" style="color: var(--cinza2); width: 90%; white-space: nowrap; text-overflow: ellipsis; overflow: hidden;">
            ...
            <section id="crew">
                ...
                <p>
                    Writers:
                    <span style="color: var(--cinza2);">{{ data.title.writers ? extenseWriting(data.title.writers) : "Information not available" }}</span>
                </p>
                <p>
                    Producers:
                    <span style="color: var(--cinza2);">{{ data.title.producers ? extenseWriting(data.title.producers) : "Information not available" }}</span>
                </p>
                <p>
                    Directors:
                    <span style="color: var(--cinza2);">{{ data.title.directors ? extenseWriting(data.title.directors) : "Information not available" }}</span>
                </p>
            </section>
            <hr class="mt-5 mb-5" />
            <div v-if="data.title.seasons > 0">
                <section id="seasons">
                    ...
                    <div class="d-flex num-seasons" style="gap: 25px;">
                        <p class="title-season d-flex justify-content-center align-items-center" :class="selectedSeason == i ? 'active' : ''" v-for="i in data.title.seasons" :key="i" @click="selectedSeason = i">
                            {{ i }}
                        </p>
                    </div>
                    ...
                    (ep) => ep.season == selectedSeason
                      ...
                      "
                                    ...
                                    <div class="tile__details">
                                        <div class="tile__title"><strong>{{ episodio.episode }}</strong>&nbsp;&nbsp;&nbsp;"{{ episodio.title }}"</div>
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                <div class="col-md-8">
                    <section id="comments">
                        ...
                        <div class="other-comments mt-5" id="comment">
                            <div class="card-comment d-flex mb-3" style="gap: 15px;" v-for="(comment, i) in data.title.comments" :key="i">
                                <div
                                    :style="
                    'background-image: url(' + comment.user_id.avatar + ');'
                  "
                                    style="min-width: 50px; height: 50px; background-repeat: no-repeat; background-size: cover; background-position: center; border-radius: 50%;"
                                ></div>
                                <div class="details d-flex flex-column">
                                    <p id="comment-author">
                                        {{ comment.user_id.first_name + " " + comment.user_id.last_name }}
                                        <span>
                                            {{ new Date(comment.date).getDate() + "/" + (parseInt(new Date(comment.date).getMonth()) + 1) + "/" + new Date(comment.date).getFullYear() + " at " + new Date(comment.date).getHours() + ":" +
                                            String(new Date(comment.date).getMinutes()).padStart( 2, "0" ) + "h" }}
                                        </span>
                                    </p>
                                    <p v-if="comment.spoiler" class="comment-message" style="color: var(--vermelho);">
                                        Spoiler!!!
                                        <span @click="mostrarSpoiler(comment.comment, i)" style="cursor: pointer;">See hidden comment</span>
                                    </p>
...
export default {
  ...
  methods: {
    ...mapActions(["getTitle", "addFavourite", "removeFavourite", "addSeen", "removeSeen", "removeCommentById", "addComment","getRating", "changeRatings", "addRatings","deleteRatings"]),
    waitForElm(selector) {
    ...
    async removeComment(id_comment_a) {
     const question=await this.$swal({
        title: 'Warning!',
        ...
        if (question.isConfirmed) {
          try {
              let response = await this.removeCommentById({ imdb_id: this.data.title.imdb_id, title_id:this.data.title._id, _id: id_comment_a});
              
              if (response.success) {
                this.data.title.comments=this.data.title.comments.filter(title=>title._id!=id_comment_a)
    ...
    async addNewComment() {
      try {
          ...
          let response = await this.addComment({ id_user: this.getLoggedUserData.data._id, imdb_id: this.data.title.imdb_id, id_title: this.data.title._id, comment: this.comentario, spoiler:this.spoiler});
    ...
    async verifyRating(){
      try {
          ...
          let response = await this.getRating({user_id: this.getLoggedUserData.data._id, imdb_id: this.data.title.imdb_id });
          if (response.success) {
      ...
      try {
        //Caso já tenha um ratting e queira alterar
         if(this.savedStars > 0){
            ...
            if (this.savedStars==0) {
              let response = await this.deleteRatings({ id_user: this.getLoggedUserData.data._id, imdb_id: this.data.title.imdb_id, id_title: this.data.title._id, rating: this.savedStars});
              ...
              let response = await this.changeRatings({ id_user: this.getLoggedUserData.data._id, imdb_id: this.data.title.imdb_id, id_title: this.data.title._id, rating: this.savedStars});
          ...
          //Caso queira adicionar um rating
            ...
            this.fillStars(n, event_type)
            let response = await this.addRatings({ id_user: this.getLoggedUserData.data._id, imdb_id: this.data.title.imdb_id, id_title: this.data.title._id, rating: this.savedStars});
    ...
    async addNewSeen() {
      try {
        if(!this.userSeen.some(filme=>filme==this.$route.params.imdbid)){
          ...
          let response = await this.removeSeen({ id: this.getLoggedUserData.data.id, imdb_id: this.data.title._id });
