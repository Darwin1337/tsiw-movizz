      ...
      <div class="container h-100">
        <div class="row h-100 w-100 pt-5 pb-5 m-0">
          ...
          <div class="main-info col-lg-8">
            ...
            <p id="title-genres">{{ extenseWriting(data.title.genres.map(a => a.genre_id.description)) }}</p>
            <div>
            ...
            <div class="mb-3 d-flex align-items-center" style="gap: 5px">
              <img style="width: 20px; min-width: 20px" :src="'https://countryflagsapi.com/svg/' + (data.title.country == 'UK' ? 'GB' : data.title.country)" />
              &nbsp;
            ...
            <p id="title-availability">{{data.title.platforms[0].platform_id.name == "Currently in cinemas" ? "" : "Available on "}}<strong>{{ extenseWriting(data.title.platforms.map(a => a.platform_id.name)) }}</strong></p>
            <div class="title-actions-wrapper d-flex justify-content-between flex-wrap">
              <div>
                <p class="orange-btn text-center mb-3">
                  IMDb
                  {{ parseFloat(data.title.imdb_rating).toFixed(1) }}
                </p>
                <p class="blur-btn text-center">Movizz {{ parseFloat(data.title.movizz_rating).toFixed(1) }}</p>
              </div>
              ...
              <div class="card-comment d-flex mb-3" style="gap: 15px" v-for="(comment, i) in data.title.comments" :key="i">
                ...
                <div class="details d-flex flex-column">
                  <p id="comment-author">
                    ...
                    <span>
                      {{ new Date(comment.date).getDate() + "/" + (parseInt(new Date(comment.date).getMonth()) + 1) + "/" + new Date(comment.date).getFullYear() + " at " + new Date(comment.date).getHours() + ":" + String(new Date(comment.date).getMinutes()).padStart(2, "0") + "h" }}
                    </span>
                  </p>
                  ...
                  <p v-else class="comment-message">
                    {{ comment.comment }}
                  </p>
                </div>

                <span v-if="comment.user_id.id == getLoggedUserData.data.id || getLoggedUserData.data.is_admin" @click="removeComment(comment._id)" class="unblock-button">
                  <i class="fas fa-trash"></i>
                </span>
              </div>
              <div>
                <p v-if="data.title.comments.length == 0">Be the first to comment on this title</p>
              </div>
            </div>
          </section>
        </div>
...
export default {
  ...
  methods: {
    ...
    async getTitleInfo() {
      ...
      try {
        this.data.title = await this.getTitle({
    ...
    async removeComment(id_comment_a) {
      ...
      if (question.isConfirmed) {
        try {
          let response = await this.removeCommentById({
            imdb_id: this.data.title.imdb_id,
            title_id: this.data.title._id,
            _id: id_comment_a,
          });
    ...
    async addNewComment() {
      try {
        let response = await this.addComment({
          id_user: this.getLoggedUserData.data._id,
          imdb_id: this.data.title.imdb_id,
          id_title: this.data.title._id,
        ...
        if (response.success) {
          ...
          this.data.title.comments.push({
            ...
            user_id: {
              avatar: this.getLoggedUserData.data.avatar,
              first_name: this.getLoggedUserData.data.first_name,
              last_name: this.getLoggedUserData.data.last_name,
              id: this.getLoggedUserData.data.id,
              _id: this.getLoggedUserData.data._id
            },
            comment: this.comentario,
            spoiler: this.spoiler
          });
    ...
    async verifyRating() {
      try {
        let response = await this.getRating({
          user_id: this.getLoggedUserData.data._id,
          imdb_id: this.data.title.imdb_id,
        });
        if (response.success) {
    ...
    async changeRating(n, event_type) {
      try {
        ...
        res.success ? res = res.msg : this.$router.push({name: "Error", params: { 0: "error" } });
        this.data.title.movizz_rating = res.movizz_rating;
      } catch (e) {
        this.$swal("Error!", e.message, "error");
      }
    },
    ...
    async addNewSeen() {
      try {
        if (!this.userSeen.some((filme) => filme == this.$route.params.imdbid)) {
          let response = await this.addSeen({
            id: this.getLoggedUserData.data.id,
    ...
    async addNewLike() {
      try {
        if (!this.userLikes.some((filme) => filme == this.$route.params.imdbid)) {
          let response = await this.addFavourite({
            id: this.getLoggedUserData.data.id,
            imdb_id: this.data.title._id,
          });
